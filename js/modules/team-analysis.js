...
import { getHistoricalTeams, getHistoricalTeamMatches, normalizeName } from "../services/historical-data.js";
...
function previousYears() {
  // El visor histórico consulta tres temporadas completas anteriores a la actual.
...
function matchRow(match, teamAId, teamBId) {
  ...
  const hg = num(match?.score?.fullTime?.home);
  const ag = num(match?.score?.fullTime?.away);
  ...
  for (const season of previousYears()) {
    try {
      ...
      for (const match of byId.values()) {
        ...
        if (row && !rows.some(r => r.id === row.id)) rows.push({...row, season});
      }
    } catch (error) {
...
function selector(teamA, teamB) {
  return `<div class="section-title">
    <div><h2>Historial entre equipos</h2><span class="muted">Busca dos clubes, incluso si pertenecen a ligas diferentes.</span></div>
    <span class="badge">Visor histórico</span>
  </div>
  <div class="card selector-card">
    <div class="team-selector-grid">
      <div>
        <label><span>Competición del equipo A</span>
          <select id="analysisLeagueA">${LEAGUES.map(([v,n]) => `<option value="${v}" ${teamA?.league===v?'selected':''}>${n}</option>`).join("")}</select>
        </label>
        <label style="display:block;margin-top:14px"><span>Equipo A</span>
          <input id="analysisSearchA" list="teamsListA" value="${esc(teamA?.name||"")}" placeholder="Buscar equipo..." autocomplete="off">
          <datalist id="teamsListA"></datalist>
        </label>
      </div>
      <div class="selector-vs">VS</div>
      <div>
        <label><span>Competición del equipo B</span>
          <select id="analysisLeagueB">${LEAGUES.map(([v,n]) => `<option value="${v}" ${teamB?.league===v?'selected':''}>${n}</option>`).join("")}</select>
        </label>
        <label style="display:block;margin-top:14px"><span>Equipo B</span>
          <input id="analysisSearchB" list="teamsListB" value="${esc(teamB?.name||"")}" placeholder="Buscar equipo..." autocomplete="off">
          <datalist id="teamsListB"></datalist>
        </label>
      </div>
    </div>
    <p id="teamSearchStatus" class="muted small-note">Selecciona las ligas y escribe el nombre de cada equipo.</p>
    <button id="runTeamAnalysis" class="primary-btn">Consultar historial</button>
  </div>`;
}

function summaryCard(rows) {
  const total = rows.length;
  const goals = rows.reduce((s,r)=>s+r.total,0);
  const yellow = rows.map(r=>r.yellow).filter(v=>v!=null);
  const red = rows.map(r=>r.red).filter(v=>v!=null);
  const aWins = rows.filter(r => r.home === rows[0]?.home ? false : false).length;
  return `<div class="grid">
    <div class="card"><small>Enfrentamientos encontrados</small><h2>${total}</h2><p class="muted">Solo partidos históricos que la fuente devuelve para ambos clubes.</p></div>
...
function renderMatches(rows) {
  if (!rows.length) return `<div class="empty"><strong>No se encontraron enfrentamientos históricos.</strong><br><span class="muted">Esto significa que no hay partidos coincidentes en las tres temporadas consultadas o que la fuente no los tiene registrados.</span></div>`;
...
export async function teamAnalysisView(leagueA="PL",teamAName="",leagueB="PL",teamBName="") {
  ...
  try {
    ...
    return base + `<div class="analysis-header-card card"><span class="badge live">Historial puro</span><div class="match-title"><h2>${esc(teamA.name)}</h2><div class="vs-big">VS</div><h2>${esc(teamB.name)}</h2></div><p class="muted">${esc(leagueName(leagueA))} · ${esc(leagueName(leagueB))}. No se generan pronósticos ni recomendaciones: solo se muestran antecedentes históricos encontrados.</p></div>${summaryCard(rows)}${renderMatches(rows)}<div class="card analysis-note"><h3>Sobre los datos</h3><p class="muted">La cantidad de enfrentamientos, goles y tarjetas depende de lo que la fuente histórica tenga registrado. Los campos no disponibles permanecen como “Sin datos”; no se inventan ni se estiman.</p></div>`;
  } catch (error) {
