...
const views={dashboard:["Resumen",dashboard],competitions:["Competiciones",competitionsView],matches:["Partidos",matchesView],teams:["Equipos",teamsView],standings:["Clasificaciones",standingsView],historical:["Historial histórico",historicalView],analysis:["Análisis del partido",matchDetail]};
...
async function render(view="dashboard",param=""){
 const v=views[view]||views.dashboard;title.textContent=v[0];
 const rendered=view==="matches"?v[1](param||"all"):view==="standings"?v[1](param||"premier-league"):view==="analysis"?v[1](param):v[1]();
...
app.addEventListener("click",e=>{const comp=e.target.closest(".comp-open");if(comp){location.hash="matches/"+comp.dataset.competition;return;}const match=e.target.closest(".match-open");if(match){location.hash="analysis/"+match.dataset.matchId;return;}const back=e.target.closest(".back-to-matches,.back-link");if(back)location.hash="matches";});
app.addEventListener("change",e=>{if(e.target.id==="matchCompetition")location.hash="matches/"+e.target.value;if(e.target.id==="standingCompetition")location.hash="standings/"+e.target.value;});
